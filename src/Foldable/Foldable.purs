...
class Foldable f where
  foldr :: forall a b. (a -> b -> b) -> b -> f a -> b   -- foldr (+) acc [1, 2, 3] = 1 + (2 + (3 + acc))
  foldl :: forall a b. (b -> a -> b) -> b -> f a -> b   -- foldl (+) acc [1, 2, 3] = ((acc + 1) + 2) + 3
  foldM :: forall a m. Monoid m => (a -> m) -> f a -> m -- map `f` onto a Foldable `ls` accumulating in a Monoid `m`
