...
--}
type StrBus
  = BusRW String -- `BusRW :: Bus ( read :: Cap, write :: Cap)` is a row type. It reads and writes strings.
...
runFiberM config =
  flip runStateT { count: 10 } -- flip so that we pass `StateT  sT` later
    <<< flip runReaderT config -- flip so that we pass `ReaderT rT` later
